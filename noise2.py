...
st.set_page_config(page_title="Noise Equations", layout="wide")
...
if selected_equation == "Sound Intensity Level":
    ...
    Io = st.number_input("Enter reference sound intensity (Io):", value=0.000000000001, format="%.12f")
    if I and Io:
...
if selected_equation == "Sound Power Level":
    ...
    Wo = st.number_input("Enter reference sound power (Wo):",value=0.000000000001, format="%.12f")
    if W and Wo:
...
if selected_equation == "TWA":
    ...
    if DS:
        TWA_calc = 16.61 * np.log10(DS / 100) + 90
